...
import { Card, Statistic, Image, Button } from 'semantic-ui-react';
...
export default class MultiReport extends PureComponent {
  static propTypes = {
  ...
  render() {
    ...
    const { quests, ap, bp } = sols[solIndex || 0];
    ...
    return ([
      ...
      <Button.Group key="nav" style={{ marginBottom: 16 }}>
        {
          sols.map((sol, index) => (
            <Button
              key={shortid.generate()}
              primary={(check.undefined(solIndex) && index === 0) || solIndex === index}
              onClick={onSolUpdate(index)}
            >
              <FormattedMessage id="Event.solution" />{` #${index + 1}`}
            </Button>
          ))
        }
      </Button.Group>,
